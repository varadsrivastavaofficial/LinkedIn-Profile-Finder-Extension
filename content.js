...
(() => {
  ...
  function detectInstitution() {
    ...
    // Fallback: use the site hostname (cleaned up)
    const host = location.hostname.replace(/^www\./, "");
    // Extract meaningful part (e.g., "iitb.ac.in" → "iitb")
  ...
  function scanForNames(root = document.body) {
    ...
    // Also run a heuristic scan on headings that might contain names
    ...
    for (const h of headings) {
      ...
      const parent = h.closest(
        '[class*="faculty"], [class*="profile"], [class*="people"], ' +
        '[class*="staff"], [class*="team"], [class*="member"], ' +
        '[class*="directory"], [class*="card"], [class*="person"], ' +
        '[class*="author"], [class*="researcher"], [class*="professor"]'
      );
      if (parent) {
        injectButton(h);
      }
